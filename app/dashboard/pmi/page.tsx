  ...
  return (
    <main className="max-w-7xl mx-auto p-6 lg:p-8 space-y-6">
      <header className="flex flex-wrap gap-4 justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-red-700 to-rose-500 bg-clip-text text-transparent">Dashboard PMI</h1>
...
function ScheduleRow({ schedule, onChange }: { schedule: Schedule; onChange: () => void }) {
  ...
  async function submitComplete(e: React.FormEvent) {
    e.preventDefault();
    const bags = parseInt(completeForm.bagCount, 10);
    if (!Number.isInteger(bags) || bags < 1) {
      toast.error("Jumlah kantong minimal 1");
      return;
    }
    setCompleting(true);
    const res = await api(`/pmi/schedules/${schedule.id}/complete`, {
      method: "POST",
      body: JSON.stringify({
        bagCount: bags,
        component: completeForm.component,
        volumeMl: parseInt(completeForm.volumeMl, 10) || 450,
    ...
    if (res.ok) {
      ...
      onChange();
    } else toast.error(typeof data.error === "string" ? data.error : "Gagal menyelesaikan donasi");
  }
  ...
  async function confirmSchedule(status: "CONFIRMED" | "REJECTED") {
    ...
    if (res.ok) {
      toast.success(`Jadwal di-${status.toLowerCase()}`);
      onChange();
  ...
  const eligBadge =
    schedule.isEligible === true ? (
      <span className="bg-emerald-100 text-emerald-700 border border-emerald-300 text-xs px-2.5 py-1 rounded-full font-bold flex items-center gap-1">✓ Layak Donor</span>
    ) : schedule.isEligible === false ? (
      <span className="bg-red-100 text-red-700 border border-red-300 text-xs px-2.5 py-1 rounded-full font-bold flex items-center gap-1">✕ Tidak Layak</span>
    ) : (
      <span className="bg-slate-100 text-slate-600 border border-slate-300 text-xs px-2.5 py-1 rounded-full font-bold flex items-center gap-1">⌛ Belum Dievaluasi</span>
    );
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition">
      ...
      <div className="p-4 flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className="w-12 h-12 bg-gradient-to-br from-red-500 to-red-700 text-white rounded-xl flex items-center justify-center font-bold shadow-sm shrink-0">{golongan}</div>
          <div>
            <p className="font-bold text-slate-900">{d.user.name}</p>
            <p className="text-xs text-slate-500">
              {d.user.email} · {d.user.phoneNum}
            </p>
      ...
      <div className="bg-slate-50 border-t border-slate-100 px-4 py-2 flex flex-wrap gap-2">
        <button type="button" onClick={() => setShowScreening(!showScreening)} className="text-xs font-semibold text-slate-700 hover:text-red-600 flex items-center gap-1 px-2 py-1 rounded hover:bg-white transition">
        ...
        {!schedule.checkup ? (
          <button type="button" onClick={() => setShowCheckup(!showCheckup)} className="text-xs font-semibold text-red-700 hover:text-red-900 flex items-center gap-1 px-2 py-1 rounded hover:bg-white transition">
            {showCheckup ? "▼" : "▶"} + Input Cek Fisik
          </button>
        ) : (
          <button type="button" onClick={() => setShowCheckup(!showCheckup)} className="text-xs font-semibold text-slate-700 hover:text-red-600 flex items-center gap-1 px-2 py-1 rounded hover:bg-white transition">
        ...
        {schedule.status === "PENDING" && schedule.isEligible === true && (
          <Button size="sm" variant="success" onClick={() => confirmSchedule("CONFIRMED")}>
            Confirm
          </Button>
        )}
        {schedule.status === "PENDING" && (
          <Button size="sm" variant="ghost" onClick={() => confirmSchedule("REJECTED")}>
            Reject
          </Button>
        )}
        {schedule.status === "CONFIRMED" && (
          <Button size="sm" variant="success" onClick={() => setShowComplete(!showComplete)}>
            🩸 Selesaikan Donasi
          </Button>
        )}
        {schedule.status === "COMPLETED" && <span className="text-xs text-emerald-600 font-semibold px-2 py-1">✓ Donasi Selesai</span>}
      </div>
        ...
        <div className="border-t border-slate-100 p-4 bg-amber-50/40">
          ...
          {!schedule.screening ? (
            ...
            <>
              ...
              {schedule.screening.details && <p className="text-xs text-slate-600 mt-2 italic">Catatan: {schedule.screening.details}</p>}
              <p className="text-xs mt-2">
                Hasil: {schedule.screening.passed ? <strong className="text-emerald-700">✓ Lolos Skrining</strong> : <strong className="text-red-700">✗ Tidak Lolos Skrining</strong>}
                <span className="text-slate-400 ml-2">· {new Date(schedule.screening.answeredAt).toLocaleString("id-ID")}</span>
              </p>
            </>
          )}
        </div>
          ...
          {schedule.checkup ? (
            <div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                ...
                <VitalItem
                  label="Tekanan Darah"
                  value={`${schedule.checkup.systolicBP}/${schedule.checkup.diastolicBP}`}
                  ok={schedule.checkup.systolicBP >= 100 && schedule.checkup.systolicBP <= 160 && schedule.checkup.diastolicBP >= 60 && schedule.checkup.diastolicBP <= 100}
                />
              ...
              {schedule.checkup.notes && <p className="text-xs text-slate-600 mt-2 italic">Catatan: {schedule.checkup.notes}</p>}
              <p className="text-[10px] text-slate-400 mt-2">Diperiksa: {new Date(schedule.checkup.examinedAt).toLocaleString("id-ID")}</p>
              {schedule.eligibilityReason && <div className="mt-2 bg-red-50 border border-red-200 rounded-lg p-2 text-xs text-red-700">Alasan tidak layak: {schedule.eligibilityReason}</div>}
            </div>
          ) : (
            <form onSubmit={submitCheckup} className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" size="sm" variant="ghost" onClick={() => setShowCheckup(false)}>
                  Batal
                </Button>
              </div>
            </form>
      ...
      {/* Expandable: Selesaikan Donasi */}
      {showComplete && schedule.status === "CONFIRMED" && (
        <div className="border-t border-slate-100 p-4 bg-emerald-50/40">
          <p className="text-xs font-semibold text-slate-600 uppercase mb-2">🩸 Selesaikan Donasi — {d.user.name}</p>
          <p className="text-xs text-slate-500 mb-3">Konfirmasi pengambilan darah. Darah otomatis masuk stok PMI Anda ({golongan}) & tercatat di Riwayat Donor pendonor.</p>
          <form onSubmit={submitComplete} className="space-y-3">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              <div>
                <label className="block text-[10px] font-semibold text-slate-700 uppercase mb-1">Jumlah Kantong</label>
                <input
                  type="number"
                  min={1}
                  step={1}
                  value={completeForm.bagCount}
                  required
                  onChange={(e) => setCompleteForm({ ...completeForm, bagCount: e.target.value })}
                  className="w-full border border-slate-300 px-2 py-1.5 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-700 uppercase mb-1">Komponen</label>
                <select
                  value={completeForm.component}
                  onChange={(e) => setCompleteForm({ ...completeForm, component: e.target.value })}
                  className="w-full border border-slate-300 px-2 py-1.5 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                >
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-700 uppercase mb-1">Volume (mL)</label>
                <input
                  type="number"
                  min={100}
                  step={50}
                  value={completeForm.volumeMl}
                  onChange={(e) => setCompleteForm({ ...completeForm, volumeMl: e.target.value })}
                  className="w-full border border-slate-300 px-2 py-1.5 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                />
              </div>
            </div>
            <div>
              ...
              <textarea
                ...
                className="w-full border border-slate-300 px-3 py-2 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none resize-none"
                placeholder="Catatan petugas…"
              />
            </div>
            <div className="flex gap-2">
              <Button type="submit" loading={completing} size="sm" variant="success" icon={<Icons.Check />}>
                Konfirmasi & Tambah ke Stok
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => setShowComplete(false)}>
                Batal
              </Button>
            </div>
          </form>
...
function ScreenItem({ label, value }: { label: string; value: boolean }) {
  return (
    <div className={`px-2 py-1.5 rounded-lg border text-[11px] flex items-center gap-1.5 ${value ? "bg-red-50 border-red-200 text-red-700" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
      <span>{value ? "✗" : "✓"}</span>
...
function BroadcastForm({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
  ...
  return (
    <Card
      title="📢 Broadcast Permintaan Stok ke Donor"
      subtitle="Donor di kota yang sama dengan PMI Anda akan menerima notifikasi"
      icon={<Icons.Heart />}
      variant="highlight"
      action={
        <Button variant="ghost" size="sm" icon={<Icons.X />} onClick={onCancel}>
          Tutup
        </Button>
      }
    >
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-900">
          ℹ️ Sistem akan kirim notif ke <strong>semua donor di kota PMI Anda</strong> yang golongannya cocok (exact match atau donor universal O−). Donor bisa langsung daftar jadwal di PMI Anda.
        </div>
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <FormField label="Lokasi Penyimpanan" className="lg:col-span-2">
            <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className={inputCls} placeholder="Gudang Pusat PMI" required />
          </FormField>
          <FormField label="Source (opsional)">
            <input value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} className={inputCls} placeholder="UTD PMI" />
          </FormField>
        </div>

        <div className="bg-white/60 border border-emerald-200 rounded-lg p-3 text-xs text-slate-700">
          ℹ️ Stok akan tersimpan dengan status <code className="bg-green-100 px-1.5 py-0.5 rounded">AVAILABLE</code> dan langsung bisa dipakai untuk fulfill request pasien.
        </div>
